...
def get_all_subjects_from_db(
    ...
    if not current_user_in_db.role == "admin":
        raise HTTPException(status_code=401, detail="Only admin get data")
    ...
    if len(subjects_lst) == 0:
        raise HTTPException(status_code=404, detail="Courses don't found")
    return subjects_lst
...
def create_new_course_in_db(
    *,
    ...
    if  current_user_in_db.role != "admin":
        raise HTTPException(status_code=401, detail="Only admin can create new course")
    ...
    if data.teacher_id not in lecturer_lst:
        raise HTTPException(
            status_code=403, detail="Only lecturer can add to the course"
        )
    ...
    if data.teacher_id not in active_lecturer_lst:
        raise HTTPException(status_code=404, detail="Lecturer don't found")
    deleted_course = (
        db.query(Course)
        .filter(
    ...
    if deleted_course:
        ...
        db.commit()
        return {"Message": "Course was created successfully"}
    if (
        db.query(Course)
        .filter(
            Course.subject_name == data.subject_name,
